                ...
                if (assignmentUsed && assignmentUsed->act) {
                    ...
                    if (currEff.size) {
                        ...
                        breakApart(currEff.variables, currEff.weights, currEff.size, currEff.constant,
                                   ...
                                   previousFluentLayer, RPGBuilder::getOpMinDuration(toUse->actID,0), RPGBuilder::getOpMaxDuration(toUse->actID,0), subTerms);
                                        ...
                                        if (tItr->first >= FluentLayerEntry::pneCount ? (minEff < 0 || maxEff < 0) : (minEff > 0 || maxEff > 0)) {
                                            ...
                                            if (tItr->first >= FluentLayerEntry::pneCount) {
                                                if (minEff < maxEff) {
                                                    ...
                                                    if (debug) {
                                                        cout << "          NB. Min eff of " << minEff << " was used\n";
                                                    }
                                                } else {
                                                    localEff = maxEff;
                                                    ...
                                                    used = &(maxUsed);
                                                    if (debug) {
                                                        cout << "          NB. Max eff of " << maxEff << " was used\n";
                                                    }
                                                }
                                            } else {
                                                if (minEff > maxEff) {
...
//    static vector<double> earliestNumericPrePOTimes;
    ...
    void recordFactLayerZero(BuildingPayload * const payload) {
        ...
        if (RPGBuilder::modifiedRPG) {
            ...
            for (; stateItr != stateEnd; ++stateItr) {
                const int factID = FACTA(stateItr);
                EpsilonResolutionTimestamp * achievedTS;
                #ifdef POPF3ANALYSIS
                achieverDetails[factID].push_back(CostedAchieverDetails(currentCosts,true));
                achievedTS = &(achieverDetails[factID].back().layer);
                #else
                achievedTS = &((*achievedInLayer)[factID]);
                *achievedTS = EpsilonResolutionTimestamp::zero();
                #endif
            ...
            if (insLayer > EpsilonResolutionTimestamp::zero()) {
                    ...
                    const int currActID = liItr->first;
                    {
                        ...
                        for (; invItr != invEnd; ++invItr) {
                            ...
                            for (; affItr != affEnd; ++affItr) {
                                if (affItr->second == VAL::E_AT) {
                                    continue;
                                }
                                if (RPGBuilder::rogueActions[affItr->first]) {
                                    continue;
                                }
                ...
                if (!affectedStarts.empty() || !affectedEnds.empty()) {
                    ...
                    payload->factLayers[insLayer].endOfJustApplied = &(setsToForbid.back());
                    {
                        ...
                        for (; sItr != sEnd; ++sItr) {
                            assert(!RPGBuilder::rogueActions[*sItr]);
    ...
    void addTILsBeforeExpansion(const int & nextTIL, FactLayerMap & factLayers, const EpsilonResolutionTimestamp & stateTS, const double & tilFrom) {
            ...
            for (; effItr != effEnd; ++effItr) {
                ...
                for (; ioItr != ioEnd; ++ioItr) {
                    ...
                    if (ioItr->second == VAL::E_AT_START) {
                        ...
                        const EpsilonResolutionTimestamp minDur(RPGBuilder::getOpMinDuration(actID, 0),false);
                        if (latestEndAllowed[actID] > thisTS + minDur) {
                            latestEndAllowed[actID] = thisTS + minDur;
                            if (evaluateDebug) {
                        ...
                        if (latestEndAllowed[actID] > thisTS) {
                            latestEndAllowed[actID] = thisTS;
                            if (evaluateDebug) {
                                cout << "Latest end of " << *(RPGBuilder::getInstantiatedOp(actID)) << " is " << latestEndAllowed[actID] << " due to TIL ¬" << *(RPGBuilder::getLiteral(*effItr)) << " at " << thisTS << endl;
                            }
                        }
        ...
        for (int a = 0; a < actCount; ++a) {
            {
                ...
                if (latestStartAllowed[a] < EpsilonResolutionTimestamp::zero() ) {
                    if (Globals::globalVerbosity & 64) {
                        cout << "Latest end of " << *(RPGBuilder::getInstantiatedOp(a)) << " can be is undefined - start cannot be early enough\n";
                    }
                    latestEndAllowed[a] = EpsilonResolutionTimestamp::undefined();
                }
            }
            {
                ...
                if (firmUpper != DBL_MAX) {
                    ...
                    if (Globals::globalVerbosity & 64) {
                        cout << "Latest end of " << *(RPGBuilder::getInstantiatedOp(a)) << " can be is " << latestEndAllowed[a] << " due to its global bound and the state timestamp of " << stateTS << endl;
                    }
                }
    ...
    void extractRP(BuildingPayload * const payload, int & h, list<pair<double, list<ActionSegment> > > & relaxedPlan, pair<int, VAL::time_spec> & earliestTIL, double & makespanEstimate) {
...

        {
            ...
            for (; gsItr != gsEnd; ++gsItr) {
                ...
                #ifdef POPF3ANALYSIS
                const EpsilonResolutionTimestamp & insLayer = achieverDetails[currGoal].back().layer;
                if (insLayer > EpsilonResolutionTimestamp::zero()) {
    ...
    //                  }
            ...
            if (!currGAL.empty()) {
                ...
                while (!currGAL.empty()) {
                    const map<int, EpsilonResolutionTimestamp>::iterator nta = currGAL.begin();
    ...
    bool applyEndEffectNow(BuildingPayload * const payload, const int & currAct, const bool & openAct, const EpsilonResolutionTimestamp & factLayerTime);
...
//  MILPRPG(theState.second);
    ...
    d->addTILsBeforeExpansion(nextTIL, payload->factLayers, EpsilonResolutionTimestamp(stateTS,true), tilFrom);
        ...
        if (!RPGBuilder::modifiedRPG && RPGBuilder::sortedExpansion) {
            ...
            if (evaluateDebug) cout << "Considering " << theState.first.size() << " initial propositional facts\n";
            {
                ...
                for (; stateItr != stateEnd; ++stateItr) {
                    ...
                    #ifdef POPF3ANALYSIS
                    const EpsilonResolutionTimestamp & layer = d->achieverDetails[factID].back().layer;
                    #else
    ...
    while (!nextHappening.empty()) {
        ...
        if (nextHappening.endActionsAppearingAtThisTime != payload->endActionsAtTime.end()) {
            ...
            const map<EpsilonResolutionTimestamp, list<int> >::iterator & eaatItr = nextHappening.endActionsAppearingAtThisTime;
        ...
        if (nextHappening.newFactsAtThisTime != payload->factLayers.end()) {
            ...
            if (currFactLayerItr->second.endOfJustApplied) {
...

                {
                    ...
                    for (; fItr != fEnd; ++fItr) {
                        assert(!RPGBuilder::rogueActions[*fItr]);
                        ...
                        if (!(--(nrItr->second))) {
                            ...
                            if (!payload->startPreconditionCounts[*fItr]) {
                                ...
                                if (evaluateDebug) cout << "-Start of " << *(RPGBuilder::getInstantiatedOp(*fItr)) << " is no longer forbidden, but its preconditions are not yet true\n";
                            }
                        }
                    }
                }
                    ...
                    for (; fItr != fEnd; ++fItr) {
                        ...
                        if (!(--(nrItr->second))) {
                            ...
                            if (!payload->endPreconditionCounts[*fItr]) {
                                if (evaluateDebug) cout << "-End of " << *(RPGBuilder::getInstantiatedOp(*fItr)) << " is no longer forbidden, and its preconditions are true\n";
                                d->noLongerForbidden.push_back(pair<int, VAL::time_spec>(*fItr, VAL::E_AT_END));
                                spawn = true;
                            } else {
                                if (evaluateDebug) cout << "-End of " << *(RPGBuilder::getInstantiatedOp(*fItr)) << " is no longer forbidden, but its preconditions are not yet true\n";
                            }
                        } else {
                            if (evaluateDebug) cout << "-End of " << *(RPGBuilder::getInstantiatedOp(*fItr)) << " is still forbidden\n";
                        }
                    }
                }
                    ...
                    for (; aItr != aEnd; ++aItr) {
                        if (aItr->second == VAL::E_AT_START) {
                            if (payload->startActionSchedule[aItr->first].isUndefined()) {
                                ...
                                cout << "- Start of " << aItr->first << ", " << *(RPGBuilder::getInstantiatedOp(aItr->first)) << ", which was actually seen\n";
                            }
                        ...
                        } else if (aItr->second == VAL::E_AT_END) {
                            if (payload->endActionSchedule[aItr->first].isUndefined()) {
                                if (!payload->startActionSchedule[aItr->first].isUndefined()) {
...
// HACK
                const double w = (rlItr->second == VAL::E_AT_START ? Private::earliestDeadlineRelevancyStart[rlItr->first->getID()] : Private::earliestDeadlineRelevancyEnd[rlItr->first->getID()]).toDouble();
...
bool RPGHeuristic::Private::applyPropositionalEffects(Private::BuildingPayload * const payload, const int & currAct, const VAL::time_spec & currTS, const bool & openEnd, const EpsilonResolutionTimestamp & nlTime, MaxDependentInfo & POtime)
{
        ...
        if (applyResult) {
            if (applyResult == 1) {
                ...
                if (goals.find(currEff) != gsEnd) {
                    ...
                    if (updateDebug) cout << "\t\tFact " << currEff <<  ", " << *(*addEffItr) << ", is new, and is a goal, but more are to come\n";
                } else {
                    if (updateDebug) cout << "\t\tFact " << currEff <<  ", " << *(*addEffItr) << ", is new\n";
                }
            }
        } else {
...
bool RPGHeuristic::Private::checkPreconditionsAreSatisfied(const int & currAct, const VAL::time_spec & ts, const EpsilonResolutionTimestamp & layer)
{
...
bool RPGHeuristic::Private::updateActionsForNewLiteralFact(Private::BuildingPayload * const payload, const int & toPropagate, const EpsilonResolutionTimestamp & factLayerTime, const bool & decrementRemainingUnsatisfiedPreconditionCounters)
{
    ...
    map<EpsilonResolutionTimestamp, list<int> > & endActionsAtTime = payload->endActionsAtTime;
    vector<EpsilonResolutionTimestamp> & startActionSchedule = payload->startActionSchedule;
                ...
                if (decrementRemainingUnsatisfiedPreconditionCounters) {
                    ...
                    if (insistOnThisEnd) {
                        ...
                        if (factLayerTime - openEndActionSchedule[currAct] >= EpsilonResolutionTimestamp::zero()) {
                            ...
                            if (updateDebug) cout << "\tOpen end cannot appear quite yet - delayed to " << openEndActionSchedule[currAct] << "\n";
    ...
    for (; depItr != depEnd; ++depItr) {
        ...
        if (startAct) {
            ...
            if (!(--numericStartPreconditionCounts[currAct]) && !startPreconditionCounts[currAct] && forbiddenStart.find(currAct) == forbiddenStart.end() && nlTime < latestStartAllowed[currAct]) {
                ...
                {
                    const EpsilonResolutionTimestamp endTime = factLayerTime + EpsilonResolutionTimestamp(RPGBuilder::getOpMinDuration(currAct, 0),true);
...
bool RPGHeuristic::Private::applyEndEffectNow(Private::BuildingPayload * const payload, const int & currAct, const bool & openEnd, const EpsilonResolutionTimestamp & factLayerTime)
{

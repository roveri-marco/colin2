...
bool FF::checkTemporalSoundness(ExtendedMinimalState & theState, const ActionSegment & actionSeg, const int oldTIL, const double moveOn)
{
    ...
    if (ts == VAL::E_AT_START || ts == VAL::E_AT_END) {
...
Solution FF::search(bool & reachedGoal)
{
    ...
    while (!searchQueue.empty()) {
        ...
        const unique_ptr<ParentData> incrementalData(FF::allowCompressionSafeScheduler ? 0 : LPScheduler::prime(currSQI->plan, currSQI->state()->getInnerState().temporalConstraints,
                currSQI->state()->startEventQueue));
        ...
        for (; helpfulActsItr != helpfulActsEnd; ++helpfulActsItr) {
            ...
            if (helpfulActsItr->second == VAL::E_AT) {
                ...
                succ = unique_ptr<SearchQueueItem>(new SearchQueueItem(applyActionToState(tempSeg, *(currSQI->state()), currSQI->plan), true));
                ...
                succ = unique_ptr<SearchQueueItem>(new SearchQueueItem(applyActionToState(*helpfulActsItr, *(currSQI->state()), currSQI->plan), true));
                if (succ->state()) {
                ...
                if (helpfulActsItr->second == VAL::E_AT) {
                    ...
                    if (visitTheState) {
                        ...
                        for (int tn = oldTIL + 1; tn <= helpfulActsItr->divisionID; ++tn) {
                            ...
                            succ = unique_ptr<SearchQueueItem>(new SearchQueueItem(applyActionToState(tempSeg, *(TILparent->state()), TILparent->plan), true));
...
#ifndef NDEBUG
        if (currSQI->internal_matchesHeader) {
            cerr << "** This state lies on the solution trajectory given\n";
            cerr << "It should be possible to apply ";
            if (currSQI->internal_matchesHeader->time_spec == VAL::E_AT) {
                cerr << "TIL " << currSQI->internal_matchesHeader->divisionID << endl;
            } else {
                cerr << *(currSQI->internal_matchesHeader->action) << ", ";
                if (currSQI->internal_matchesHeader->time_spec == VAL::E_AT_START) {
                    cerr << "start\n";
                } else {
                    cerr << "end\n";
                }
            ...
            if (!tsSound) {
                ...
                if (helpfulActsItr->second == VAL::E_AT) {
                    ...
                    if (visitTheState) {
                        ...
                        if (Globals::globalVerbosity & 2) {
                            cout << "TIL parent is " << TILparent << endl;
                        }
                        ...
                        for (int tn = oldTIL + 1; tn <= helpfulActsItr->divisionID; ++tn) {
                            ...
                            if (TILparent != currSQI.get()) {
                                delete TILparent;
                                if (Globals::globalVerbosity & 2) {
                                    cout << "Deleting TIL parent " << TILparent << endl;
                                }
                                TILparentAutoDelete.release();
                            }
                            ...
                            TILparent = succ.release();
                            if (Globals::globalVerbosity & 2) {
                                cout << "TIL parent is now " << TILparent << endl;
                            }
                            TILparentAutoDelete = unique_ptr<SearchQueueItem>(TILparent);
...
list<FFEvent> * FF::reprocessPlan(list<FFEvent> * oldSoln, TemporalConstraints * cons)
{
    ...
    for (int stepID = 0; oldSolnItr != oldSolnEnd; ++oldSolnItr, ++stepID) {
        ...
        unique_ptr<SearchQueueItem> succ = unique_ptr<SearchQueueItem>(new SearchQueueItem(applyActionToState(nextSeg, *(currSQI->state()), currSQI->plan), true));
        succ->heuristicValue.makespan = currSQI->heuristicValue.makespan;
